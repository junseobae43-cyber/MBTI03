...
<meta charset="utf-8">
<style>
    body { background-color: #111; color: white; text-align: center; font-family: '맑은 고딕', sans-serif; margin: 0; padding: 0; user-select: none; }
    canvas { background: #222; border: 4px solid #555; display: block; margin: 10px auto; cursor: pointer; }
    .info { font-size: 14px; color: #ccc; margin-bottom: 5px; background: #222; padding: 8px; display: inline-block; border-radius: 5px; }
    .notice { color: #ffeb3b; font-weight: bold; margin-bottom: 5px; }
</style>
</head>
<body>
    <div class="notice">⚠️ 게임 화면(검은창)을 한 번 클릭한 후 키보드를 눌러주세요!</div>
    <div class="info">
        <b>[1P 조작]</b> 이동: A, D | 점프: W | 평타: F | 궁극기: G <br>
        <b>[2P 조작]</b> 이동: ←, → | 점프: ↑ | 평타: K | 궁극기: L (궁극기는 게이지 100% 시 가능)
    </div>
    <canvas id="gameCanvas" width="950" height="480" tabindex="1"></canvas>
...
(function() {
    ...
    function updatePlayer(p, enemy, left1, left2, right1, right2, jump1, jump2, atk1, atk2, ult1, ult2) {
        if (keys[left1] || keys[left2]) p.x -= p.speed;
        if (keys[right1] || keys[right2]) p.x += p.speed;

        if ((keys[jump1] || keys[jump2]) && !p.isJumping) {
            p.vy = -14;
            p.isJumping = true;
        }
        ...
        if ((keys[atk1] || keys[atk2]) && !p.attacking) {
            doAttack(p, enemy, p.atk, 65, false);
        }
        if ((keys[ult1] || keys[ult2]) && !p.attacking && p.ultGauge >= 100) {
    ...
    function drawSelectScreen() {
        ...
        ctx.fillStyle = "#FFF";
        ctx.font = "bold 22px '맑은 고딕'";
        ctx.fillText("캐릭터 선택 (1P: A/D/F선택 | 2P: ←/→/K선택)", 260, 40);
